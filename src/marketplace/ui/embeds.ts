...
export function buildCaseSummaryEmbed(input: {
  ...
  const embed = new EmbedBuilder()
    ...
    .setTitle(`${approval.kind === 'sell' ? '🟢 Πώληση / Sell' : '🔵 Αγορά / Buy'} — ${approval.title}`)
    .addFields(
...
export function buildPublicPostEmbed(input: {
  ...
  const embed = new EmbedBuilder()
    .setColor(color)
    .setTitle(`${titlePrefix}${approval.kind === 'sell' ? '🟢 Πώληση / Sell' : '🔵 Αγορά / Buy'} — ${approval.title}`);

  // --- Seller/Buyer info (πάντα φαίνονται) ---
  if (approval.kind === 'sell' && approval.requesterId) {
    embed.addFields({
      name: '👤 Seller',
      value: userMention(approval.requesterId),
    });
  }

  if (approval.kind === 'buy' && approval.requesterId) {
    embed.addFields({
      name: '🙋 Buyer',
      value: userMention(approval.requesterId),
    });
